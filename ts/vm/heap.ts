...
export function makePointer(ptr: number): number {
    bigUint64Array[0] = PTR_TAG | BigInt(ptr >>> 0);
    return float64Array[0];
}

export function isPointer(val: any): boolean {
    if (typeof val !== 'number') return false;
    float64Array[0] = val;
    return (bigUint64Array[0] & ~PTR_MASK) === PTR_TAG;
}

export function getPointerValue(val: number): number {
    float64Array[0] = val;
...
export enum ValueTag {
    ...
    UNDEFINED = 3,
    PTR = 4,
    EXTERNAL = 5 // Для замыканий и нативных жс-объектов
}
...
export class CustomHeap {
    ...
    private freeExternalRefs: number[] = [];
    private aliveExternals: Set<number> = new Set();
    private getRoots: () => any[];
    public pinnedRoots: number[] = [];

    constructor(sizeInBytes: number = 1024 * 1024 * 10, getRoots: () => any[] = () => []) {
        this.buffer = new ArrayBuffer(sizeInBytes);
        this.view = new DataView(this.buffer);
        this.getRoots = getRoots;
    }

    public allocate(size: number, type: HeapType): number {
    ...
    public allocateObject(capacity: number): number {
        // структура объекта:
    ...
    public getPropertyCached(objPtr: number, keyStr: string, cachedIndex: number): { value: any, index: number } {
        if (this.view.getUint8(objPtr) !== HeapType.OBJECT) throw new Error("Heap Error: Not an object");
    ...
    public setProperty(objPtr: number, keyStr: string, value: any): void {
        if (this.view.getUint8(objPtr) !== HeapType.OBJECT) throw new Error("Heap Error: Not an object");
    ...
    public setPropertyCached(objPtr: number, keyStr: string, value: any, cachedIndex: number): number {
        if (this.view.getUint8(objPtr) !== HeapType.OBJECT) throw new Error("Heap Error: Not an object");
        let count = this.view.getUint16(objPtr + 6, false);
        const capacity = this.view.getUint16(objPtr + 8, false);
    ...
    public getProperty(objPtr: number, keyStr: string): any {
        const count = this.view.getUint16(objPtr + 6, false);
        return this.getPropertyWithIndex(objPtr, keyStr, count).value;
    }

    private getPropertyWithIndex(objPtr: number, keyStr: string, count: number): { value: any, index: number } {
        let offset = objPtr + 10;
        for (let i = 0; i < count; i++) {
            const currentKeyPtr = this.view.getUint32(offset, false);
            if (this.readString(currentKeyPtr) === keyStr) {
    ...
    public deleteProperty(objPtr: number, keyStr: string): boolean {
        this.setProperty(objPtr, keyStr, undefined);
        return true;
    }

    public getKeys(objPtr: number): string[] {
        if (this.view.getUint8(objPtr) !== HeapType.OBJECT) throw new Error("Heap Error: Not an object");
        const count = this.view.getUint16(objPtr + 6, false);
        let offset = objPtr + 10;
        const keys: string[] = [];
    ...
    public allocateArray(capacity: number): number {
    ...
    public getArrayLength(arrayPtr: number): number {
        if (this.view.getUint8(arrayPtr) !== HeapType.ARRAY) throw new Error("Heap Error: Not an array");
        return this.view.getUint32(arrayPtr + 6, false);
    }

    public getArrayElement(arrayPtr: number, index: number): any {
        if (this.view.getUint8(arrayPtr) !== HeapType.ARRAY) throw new Error("Heap Error: Not an array");
    ...
    public setArrayElement(arrayPtr: number, index: number, value: any): void {
        if (this.view.getUint8(arrayPtr) !== HeapType.ARRAY) throw new Error("Heap Error: Not an array");
        const length = this.view.getUint32(arrayPtr + 6, false);
    ...
    public pushArrayElement(arrayPtr: number, value: any): number {
        if (this.view.getUint8(arrayPtr) !== HeapType.ARRAY) throw new Error("Heap Error: Not an array");
    ...
    public readString(ptr: number): string {
        if (this.view.getUint8(ptr) !== HeapType.STRING) throw new Error("Heap Error: Expected String");
    ...
    private mark(roots: any[]) {
        const worklist: number[] = [];
        const visitedExternals = new Set<any>();

        const scanNativeForPointers = (val: any) => {
            if (!val) return;
            if (isPointer(val) && this.isValidPointer(getPointerValue(val))) {
                worklist.push(getPointerValue(val));
            } else if (typeof val === 'object') {
                if (visitedExternals.has(val)) return;
                ...
                if (val.type === 'closure' && val.env) {
                    ...
                    while (currentEnv) {
                        if (visitedExternals.has(currentEnv)) break;
        ...
        for (const root of this.pinnedRoots) {
            if (this.isValidPointer(root)) worklist.push(root);
        }
        ...
        while (worklist.length > 0) {
            ...
            if (type === HeapType.OBJECT) {
                ...
                if (this.isValidPointer(dataPtr)) worklist.push(dataPtr);
                ...
                for (let offset = payloadStart; offset <= payloadEnd - 4; offset += 4) {
                    ...
                    if (this.isValidPointer(possiblePtr)) worklist.push(possiblePtr);
                }
            }
        }
    }
    ...
    private sweep() {
        ...
        while (ptr < this.heapTop) {
            const isMarked = this.view.getUint8(ptr + 1) === 1;
            const size = this.view.getUint32(ptr + 2, false);
            if (size === 0) throw new Error("Heap Corruption: Size 0");
    ...
    public writeTaggedValue(offset: number, value: any) {
    ...
    public readTaggedValue(offset: number): any {
