...
import { VmClosure } from './vm.types.js';
import { SuspendException, YieldException } from './exceptions.js';

export class ClosureHandler {
    constructor(private vm: VM) {}

    public callClosure(closure: VmClosure, argCount: number, basePointer: number, thisArg?: any, argsArray?: any[], isConstructorCall = false) {
        if (closure.func.isGenerator) {
            if (isConstructorCall) throw new TypeError("Error: Generator functions cannot be used as constructors.");
            ...
            for (let i = 0; i < closure.func.arity; i++) newEnv.variables[i + 1] = i < args.length ? args[i] : undefined;
            if (closure.func.hasRest) newEnv.variables[closure.func.arity + 1] = args.slice(closure.func.arity);

            childVm.stack[childVm.sp++] = closure;
            childVm.fp++;
            if (childVm.frames[childVm.fp]) {
                const f = childVm.frames[childVm.fp];
                f.closure = closure; f.env = newEnv; f.basePointer = 0; f.isConstructorCall = false; f.constructedInstance = thisArg; f.catchHandlers.length = 0; f.ip = 0;
            } else {
            ...
            if (closure.func.isAsync) {
                const asyncGeneratorObj = {
                    next: async (arg?: any) => {
                        if (isFinished) return { value: undefined, done: true };
                        if (!isFirstNext) childVm.stack[childVm.sp++] = arg;
                        isFirstNext = false;
                        while (true) {
                            try {
                                childVm.run(0);
                                ...
                                if (e instanceof SuspendException) {
                                    try {
                                        ...
                                        if (!childVm.exceptionHandler.handleException(err, 0)) throw err;
                                    }
            ...
            const generatorObj = {
                next: (arg?: any) => {
                    ...
                    try {
                        ...
                        if (e instanceof YieldException) return { value: e.value, done: false };
                        throw e;
                    }
                },
        ...
        if (closure.func.isAsync) {
            if (isConstructorCall) throw new TypeError("Error: Async functions cannot be used as constructors.");
            const args = argsArray || this.vm.stack.slice(basePointer + 1, basePointer + 1 + argCount);
            const childVm = this.vm.createChildVM();
        ...
        if (argsArray) {
            for (let i = 0; i < arity; i++) newEnv.variables[i + 1] = i < argsArray.length ? argsArray[i] : undefined;
            if (closure.func.hasRest) newEnv.variables[arity + 1] = argsArray.slice(arity);
        } else {
            for (let i = 0; i < arity; i++) newEnv.variables[i + 1] = i < argCount ? this.vm.stack[basePointer + 1 + i] : undefined;
            if (closure.func.hasRest) {
                const restCount = Math.max(0, argCount - arity);
                const restArr = new Array(restCount);
                for (let i = 0; i < restCount; i++) restArr[i] = this.vm.stack[basePointer + 1 + arity + i];
        ...
        if (this.vm.fp >= this.vm.MAX_CALL_STACK_SIZE - 1) {
            ...
            if (!this.vm.exceptionHandler.handleException(err, 0)) throw err;
            return;
        }
        ...
        if (this.vm.frames[this.vm.fp]) {
            ...
            f.closure = closure; f.env = newEnv; f.basePointer = basePointer; f.isConstructorCall = isConstructorCall; f.constructedInstance = thisArg; f.catchHandlers.length = 0; f.ip = 0;
        } else {
            this.vm.frames[this.vm.fp] = new CallFrame(closure, newEnv, basePointer, isConstructorCall, thisArg);
        }
    }

    public async executeAsyncClosure(closure: VmClosure, args: any[], thisArg?: any): Promise<any> {
        const newEnv = new Environment(closure.env);
        newEnv.variables[0] = thisArg !== undefined ? thisArg : closure;
        for (let i = 0; i < closure.func.arity; i++) newEnv.variables[i + 1] = i < args.length ? args[i] : undefined;
        if (closure.func.hasRest) newEnv.variables[closure.func.arity + 1] = args.slice(closure.func.arity);
