...
import { VmFunction, VmClosure } from './vm.types.js';
...
export class VM {
    public stack: any[] = new Array(65536);
    public frames: CallFrame[] = new Array(10000);
    public constants: any[] = [];
    public heap: CustomHeap;
    public globalEnv: Environment;
    public reverseOpcodeMap: number[] = new Array(256).fill(0);
    public fp: number = -1;
    public sp: number = 0;
    public readonly MAX_CALL_STACK_SIZE = 10000;
    public bytecode: Uint8Array;
    public pushUndefOpcode: number = 0;
    public returnOpcode: number = 0;

    public exceptionHandler: ExceptionHandler;
    public interopHandler: InteropHandler;
    ...
    private decodeChunk(chunk: Uint8Array): Int32Array {
        ...
        while (ip < chunk.length) {
            ...
            switch (opcode) {
                ...
                case Opcode.CONSOLE_ERROR:
                    ...
                    if (opcode === Opcode.PUSH_INT8 && wordcode[ip] > 127) wordcode[ip] -= 256;
                    ip++;
                    break;
    ...
    private parseBytecode(): number {
        ...
        for (let i = 0; i < 256; i++) {
            ...
            if (i === Opcode.PUSH_UNDEFINED) this.pushUndefOpcode = shuffled;
            if (i === Opcode.RETURN) this.returnOpcode = shuffled;
        }
        ...
        for (let i = 0; i < constantsCount; i++) {
            ...
            if (type === tagNum) {
                ...
                for (let j = 0; j < bigintLength; j++) bigintBytes[j] ^= biXorKey;
                ...
                for (let j = 0; j < nameLength; j++) nameBytes[j] ^= fnXorKey;
    ...
    private collectRoots(): any[] {
        const roots: any[] = [];

        for (let i = 0; i < this.sp; i++) roots.push(this.stack[i]);
        for (let i = 0; i < this.globalEnv.variables.length; i++) roots.push(this.globalEnv.variables[i]);
        for (let i = 0; i <= this.fp; i++) {
            const env = this.frames[i].env;
            if (env)
                for (let j = 0; j < env.variables.length; j++) roots.push(env.variables[j]);
        }
        return roots;
    }

    public run(targetFrameCount: number = 0): void {
        ...
        // расшифровка строк
        const resolveConst = (index: number) => {
            const c = constants[index];
            if (c && c._isEnc) {
                const b = c._encBytes;
                const d = new Uint8Array(b.length);
                const k = c._encKey;
                for (let i = 0; i < b.length; i++) d[i] = b[i] ^ k;
        ...
        while (this.fp >= targetFrameCount) {
            try {
                while (this.fp >= targetFrameCount) {
                    ...
                    switch (opcode) {
                        ...
                        case Opcode.GET_FOR_IN_ITERATOR: {
                            ...
                            function* enumerate(o: any, h: CustomHeap) {
                                if (typeof o === 'number' && isPointer(o)) {
                                    for (const k of h.getKeys(getPointerValue(o))) yield k;
                                } else if (o != null) {
                        ...
                        case Opcode.GET_FOR_OF_ITERATOR: {
                            ...
                            if (typeof obj === 'number' && isPointer(obj) && heap.view.getUint8(getPointerValue(obj)) === HeapType.ARRAY) {
                                ...
                                function* enumerateHeapArray(h: CustomHeap, p: number) {
                        ...
                        case Opcode.GET_ASYNC_ITERATOR: {
                            ...
                            if (typeof obj === 'number' && isPointer(obj) && heap.view.getUint8(getPointerValue(obj)) === HeapType.ARRAY) {
                                const ptr = getPointerValue(obj);
                                async function* enumerateHeapArrayAsync(h: CustomHeap, p: number) {
                                    const len = h.getArrayLength(p);
                                    for (let i = 0; i < len; i++) {
                                        yield h.getArrayElement(p, i);
                                    }
                                ...
                                stack[sp++] = (obj as any)[Symbol.asyncIterator]();
                            } else if (obj != null && typeof obj[Symbol.iterator] === 'function') {
                        ...
                        case Opcode.MAKE_CLOSURE: {
                            ...
                            ip += 2;
                            const func = constants[constIndex] as VmFunction;
                            env.isCaptured = true;
                        ...
                        case Opcode.STORE_VAR: {
                            ...
                            ip += 2;
                            let targetEnv = env;
                            for (let i = 0; i < hops; i++) targetEnv = targetEnv.outer!;
                        ...
                        case Opcode.CALL: {
                            ...
                            if (!func.isAsync && !func.isGenerator) {
                                ...
                                if (func.hasRest) {
                                    ...
                                    for (let i = 0; i < restCount; i++) restArr[i] = stack[calleeIndex + 1 + arity + i];
                        ...
                        case Opcode.NEW: {
                            ...
                            if (typeof callee === 'function') {
                                ...
                                const result = new(callee as any)(...nativeArgs);
                                restoreState();
                                stack[sp++] = result;
                                break;
                            }
                        ...
                        case Opcode.INVOKE: {
                            ...
                            const argCount = wordcode[ip];
                            ip++;
                            const methodName = resolveConst(nameIndex) as string;
                        ...
                        case Opcode.INVOKE_SPREAD: {
                            ...
                            if (method && method.type === 'closure') {
                                ...
                                saveState();
                                const nativeArgs = vmArgs.map((arg: any) => this.interopHandler.unwrapForNative(arg, targetFrameCount));
                        ...
                        case Opcode.GET_PROPERTY: {
                            ...
                            if (typeof obj === 'number' && isPointer(obj)) {
                                ...
                                if (type === HeapType.OBJECT) {
                                    ...
                                    if (key === 'length') value = heap.getArrayLength(objPtr);
                                    else if (typeof key === 'number' && key % 1 === 0) value = heap.getArrayElement(objPtr, key);
                        ...
                        case Opcode.GET_NAMED_PROPERTY: {
                            ...
                            ip += 2;
                            const key = resolveConst(nameIndex) as string;
                            const obj = stack[--sp];
                            let value;
                            if (typeof obj === 'number' && isPointer(obj)) {
                                const objPtr = getPointerValue(obj);
                        ...
                        case Opcode.SET_PROPERTY: {
                            ...
                            if (typeof obj === 'number' && isPointer(obj)) {
                                ...
                                if (type === HeapType.OBJECT) {
                                    ...
                                    if (typeof key === 'number' && key % 1 === 0) heap.setArrayElement(objPtr, key, value);
                                }
                            } else {
                                obj[key] = value;
                            }
                        ...
                        case Opcode.LOAD_GLOBAL: {
                            const nameIndex = wordcode[ip];
                            ip += 2;
                            const name = resolveConst(nameIndex) as string;
                            if (name in globalThis) {
                                stack[sp++] = (globalThis as any)[name];
                            } else {
                        ...
                        case Opcode.STORE_GLOBAL: {
                            ...
                            ip += 2;
                            const name = resolveConst(nameIndex) as string;
                            (globalThis as any)[name] = stack[sp - 1];
                            break;
                        }
                        ...
                        case Opcode.TYPEOF: {
                            ...
                            if (res === 'number' && isPointer(value)) res = "object";
                            else if (value && typeof value === 'object' && value.type === 'closure') res = "function";
                            stack[sp - 1] = res;
                            break;
                        }
                        ...
                        case Opcode.DELETE_GLOBAL: {
                            ...
                            ip += 2;
                            const name = resolveConst(nameIndex) as string;
                            stack[sp++] = delete(globalThis as any)[name];
                            break;
                        }
                        ...
                        case Opcode.JUMP_IF_NOT_NULLISH: {
                            ...
                            if (condition !== null && condition !== undefined) ip += offset;
                            break;
                        }
                        ...
                        case Opcode.TYPEOF_GLOBAL: {
                            ...
                            ip += 2;
                            const name = resolveConst(nameIndex) as string;
                            const globalObj = (globalThis as any)[name];
                            let res = globalObj !== undefined ? typeof globalObj : "undefined";
                            if (res === 'number' && isPointer(globalObj)) res = "object";
                            else if (globalObj && typeof globalObj === 'object' && globalObj.type === 'closure') res = "function";
                            stack[sp++] = res;
                            break;
                        }
                ...
                saveState();
                if (!this.exceptionHandler.handleException(e, targetFrameCount)) throw e; 
                restoreState(); 
            }
        }
    }

    public createChildVM(): VM {
