...
export function buildBinary(
    ...
    constantTags: { num: number, str: number, func: number, bigint: number }
): Uint8Array {
    let constantsSize = 0;
    const encodedStrings: (Uint8Array | null)[] = constants.map(c => (typeof c === 'string' ? new TextEncoder().encode(c) : typeof c === 'bigint' ? new TextEncoder().encode(c.toString()) : null));
    ...
    for (let i = 0; i < padLen; i++) result[2 + i] = Math.floor(Math.random() * 256);

    let offset = 2 + padLen;
    const hashOffset = offset;

    view.setUint32(offset, 0, false); offset += 4; // хеш (пока 0)
    view.setUint16(offset, 0, false); offset += 2; // версия
    view.setUint16(offset, constants.length, false); offset += 2; // колво констант
    ...
    result.set(opcodeMap, offset); offset += 256;
    ...
    constants.forEach((c, i) => {
      if (typeof c === 'number') {
        ...
        offset += 1;
        for (let j = 0; j < stringBytes.length; j++) result[offset + j] = stringBytes[j] ^ xorKey;
        ...
        view.setUint8(offset, xorKey);
        offset += 1;
        for (let j = 0; j < nameBytes.length; j++) result[offset + j] = nameBytes[j] ^ xorKey;
