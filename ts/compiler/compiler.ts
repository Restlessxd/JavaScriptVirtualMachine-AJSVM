...
import { CompiledFunction, CompilerLogLevel } from './compiler.types.js';
...
export class Compiler {
  ...
  private static generateOpcodeMap(): Uint8Array {
  ...
  public opcodeMap: Uint8Array;
  // отслеживаем локальные переменные и их глубину вложенности
  public locals: { name: string, depth: number }[] = [];
  public scopeDepth: number = 0;
  public functionName: string | null = null;

  public currentClass: { name: string } | null = null;
  public currentSuperClass: { name: string } | null = null;
  public currentTryDepth: number = 0;
  // стек для отслеживания джамп инструкций внутри циклов для break и continue
  public loopStack: { breakJumps: number[], continueJumps: number[], tryDepth: number, isSwitch?: boolean }[] = [];
  // короткие замыкания
  public currentChainJumps: number[][] = [];
  
  // хранит список реально использованных опкодов после оптимизации
  public usedOpcodes: Set<number>;

  public constantTags: { num: number, str: number, func: number, bigint: number };

  // добавляем конструктор для отслеживания типа компиляции (скрипт или функция)
  constructor(public type: 'script' | 'function' = 'script', constants: any[] = [], public parent: Compiler | null = null, public logLevel: CompilerLogLevel = CompilerLogLevel.NONE) {
    ...
    if (parent) {
        ...
        for (let i = 255; i > 0; i--) { const j = Math.floor(Math.random()*(i+1)); [pool[i], pool[j]] = [pool[j], pool[i]]; }
        this.constantTags = { num: pool[0], str: pool[1], func: pool[2], bigint: pool[3] };
    }
  }

  public log(level: CompilerLogLevel, ...args: any[]) {
  ...
  public compile(ast: acorn.Node): Uint8Array {
  ...
  public endCompilation(): Uint8Array {
  ...
  public declareVariable(name: string) {
  ...
  public declarePatternVariables(pattern: any) {
  ...
  public resolveVariable(name: string): { type: 'local', hops: number, index: number } | { type: 'global' } {
    this.log(CompilerLogLevel.TRACE, `Resolving variable: ${name}`);
    let hops = 0;
    let current: Compiler | null = this;
  ...
  public compileDeclarationPattern(pattern: any) {
    if (pattern.type === 'Identifier') {
      ...
      pattern.elements.forEach((element: any, index: number) => {
        if (!element) return;
  ...
  public compileParameters(params: any[]): { arity: number, hasRest: boolean } {
    let arity = 0;
    let hasRest = false;

    params.forEach((p: any) => {
        if (p.type === 'RestElement') {
            if (p.argument.type !== 'Identifier') throw new Error("Compiler error: Destructuring rest parameters are not supported yet.");
            this.declareVariable(p.argument.name);
            hasRest = true;
        } else if (p.type === 'AssignmentPattern') {
            if (p.left.type !== 'Identifier') throw new Error("Compiler error: Destructuring assignment parameters are not supported yet.");
    ...
    params.forEach((p: any) => {
        if (p.type === 'AssignmentPattern' && p.left.type === 'Identifier') {
            ...
            if (res.type !== 'local') throw new Error("Unreachable");
  ...
  public compileBlock(body: any[]) {
    // hoisting
    body.forEach(stmt => {
      if (stmt.type === 'FunctionDeclaration') {
        this.declareVariable(stmt.id.name);
      } else if (stmt.type === 'VariableDeclaration') {
        stmt.declarations.forEach((decl: any) => this.declarePatternVariables(decl.id));
  ...
  public visit(node: any) {
    if (node && node.type) this.log(CompilerLogLevel.DEBUG, `Visiting node: ${node.type}`);
    visitNode(this, node);
  }

  public compileFunctionDeclaration(node: any) {
  ...
  public addConstant(value: any): number {
  ...
  public emit16(value: number): number {
  ...
  public patch16(position: number, value: number) {
  ...
  public emitLoadVar(resolution: { hops: number, index: number }) {
  ...
  public emitStoreVar(resolution: { hops: number, index: number }) {
    if (resolution.hops === 0) {
