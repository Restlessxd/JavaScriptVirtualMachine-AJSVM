...
export function visitBlock(compiler: Compiler, node: any) {
    compiler.beginScope();
    compiler.compileBlock(node.body);
    compiler.endScope();
}

export function visitExpressionStatement(compiler: Compiler, node: any) {
    compiler.visit(node.expression);
    compiler.emit(Opcode.POP);
}

export function visitIfStatement(compiler: Compiler, node: any) {
...
export function visitWhileStatement(compiler: Compiler, node: any) {
...
export function visitDoWhileStatement(compiler: Compiler, node: any) {
    const loopStart = compiler.bytecode.length;
    compiler.loopStack.push({ breakJumps: [], continueJumps: [], tryDepth: compiler.currentTryDepth });
    compiler.visit(node.body);
    const loopInfo = compiler.loopStack.pop()!;
...
export function visitForStatement(compiler: Compiler, node: any) {
    compiler.beginScope();
    if (node.init) {
        if (node.init.type === 'VariableDeclaration') {
            node.init.declarations.forEach((decl: any) => compiler.declarePatternVariables(decl.id));
        }
...
export function visitForInOfStatement(compiler: Compiler, node: any) {
    compiler.beginScope();
    if (node.left.type === 'VariableDeclaration') {
        node.left.declarations.forEach((decl: any) => compiler.declarePatternVariables(decl.id));
    }
...
export function visitBreakStatement(compiler: Compiler, node: any) {
    if (node.label) throw new Error("Compiler error: Labeled break statements are not supported.");
    if (compiler.loopStack.length === 0) throw new Error("SyntaxError: Illegal break statement");
...
export function visitContinueStatement(compiler: Compiler, node: any) {
    if (node.label) throw new Error("Compiler error: Labeled continue statements are not supported.");
    let loopIndex = compiler.loopStack.length - 1;
    while (loopIndex >= 0 && compiler.loopStack[loopIndex].isSwitch) {
        loopIndex--;
    }
    if (loopIndex < 0) throw new Error("SyntaxError: Illegal continue statement");
...
export function visitReturnStatement(compiler: Compiler, node: any) {
    if (compiler.type !== 'function') throw new Error("SyntaxError: 'return' outside a function.");
    if (node.argument) {
...
export function visitThrowStatement(compiler: Compiler, node: any) {
    compiler.visit(node.argument);
    compiler.emit(Opcode.THROW);
}

export function visitTryStatement(compiler: Compiler, node: any) {
    ...
    if (node.handler) {
        ...
        if (node.handler.param) {
            ...
            compiler.emitStoreVar(resolution as any);
            compiler.emit(Opcode.POP);
        } else {
            compiler.emit(Opcode.POP);
        }
    ...
    if (node.finalizer) throw new Error("Compiler error: try...finally is not supported yet.");
}

export function visitSwitchStatement(compiler: Compiler, node: any) {
    compiler.beginScope();
    ...
    if (discRes.type !== 'local') throw new Error("Unreachable");
    ...
    const caseJumps: { testJumpPos: number, bodyPos?: number }[] = [];
    ...
    for (let i = 0; i < node.cases.length; i++) {
        ...
        clause.consequent.forEach((stmt: any) => compiler.visit(stmt));
    }
    const endPos = compiler.bytecode.length;
    for (let i = 0; i < node.cases.length; i++) {
