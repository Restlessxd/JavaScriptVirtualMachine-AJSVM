...
export function visitLiteral(compiler: Compiler, node: any) {
...
export function visitTemplateLiteral(compiler: Compiler, node: any) {
...
export function visitArrayExpression(compiler: Compiler, node: any) {
    const elements = node.elements;
    const hasSpread = elements.some((e: any) => e && e.type === 'SpreadElement');
    if (hasSpread) {
        ...
        for (let i = 0; i < elements.length; i++) {
            if (elements[i]) compiler.visit(elements[i]);
            else compiler.emit(Opcode.PUSH_UNDEFINED);
        }
        compiler.emit(Opcode.MAKE_ARRAY);
...
export function visitObjectExpression(compiler: Compiler, node: any) {
