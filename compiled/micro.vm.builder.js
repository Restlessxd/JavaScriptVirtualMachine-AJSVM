...
export class MicroVMBuilder {
    static async build(jsCode, vmTemplatePath, options = defaultOptions) {
        ...
        if (options.production) {
            cleanVm = cleanVm.replace(/if\s*\(\(hash\s*>>>\s*0\)\s*!==\s*expectedHash\)\s*\{\s*throw\s*new\s*Error\([^)]+\);\s*\}/g, 'if ((hash >>> 0) !== expectedHash) { return; }');
        }
        const OpcodeMap = Object.fromEntries(Object.entries(Opcode).map(([k, v]) => [v, k]));
        const usedOpcodeNames = Array.from(usedOpcodes).map(code => OpcodeMap[code]);
        ...
        requiredOpcodes.forEach(op => {
            if (!usedOpcodeNames.includes(op))
                usedOpcodeNames.push(op);
        });
        ...
        cleanVm = cleanVm.replace(/Opcode\.([A-Z0-9_]+)/g, (match, opcodeName) => {
            const numericValue = Opcode[opcodeName];
