...
export class InteropHandler {
    ...
    unwrapForNative(arg, targetFrameCount) {
        if (typeof arg === 'number' && isPointer(arg)) {
            ...
            if (type === HeapType.OBJECT) {
                ...
                return new Proxy([], {
                    get: (target, prop) => {
                        if (prop === 'length')
                            return this.vm.heap.getArrayLength(ptr);
    ...
    wrapClosureForNative(arg, targetFrameCount) {
        if (arg && typeof arg === 'object' && arg.type === 'closure') {
            return (...callbackArgs) => {
                ...
                for (let i = 0; i < callbackArgs.length; i++)
                    this.vm.stack[this.vm.sp++] = callbackArgs[i];
                const newEnv = new Environment(arg.env);
                newEnv.variables[0] = arg;
                for (let i = 0; i < arg.func.arity; i++)
                    newEnv.variables[i + 1] = callbackArgs[i];
                if (arg.func.hasRest)
                    newEnv.variables[arg.func.arity + 1] = callbackArgs.slice(arg.func.arity);
                this.vm.fp++;
                if (this.vm.frames[this.vm.fp]) {
                    const f = this.vm.frames[this.vm.fp];
                    f.closure = arg;
                    f.env = newEnv;
                    f.basePointer = currentCalleeIndex;
