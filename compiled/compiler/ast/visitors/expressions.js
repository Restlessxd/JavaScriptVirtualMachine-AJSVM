...
export function visitChainExpression(compiler, node) {
...
export function visitBinaryExpression(compiler, node) {
    if (node.left.type === 'Literal' && node.right.type === 'Literal' && typeof node.left.value === 'number' && typeof node.right.value === 'number') {
        ...
        switch (node.operator) {
            case '+':
                foldedValue = node.left.value + node.right.value;
                break;
            case '-':
                foldedValue = node.left.value - node.right.value;
                break;
    ...
    switch (node.operator) {
        ...
        case '%':
            ...
            break;
        case '**':
            compiler.emit(Opcode.EXP);
            break;
        case '&':
            compiler.emit(Opcode.BITWISE_AND);
            break;
        case '|':
            compiler.emit(Opcode.BITWISE_OR);
            break;
        case '^':
            compiler.emit(Opcode.BITWISE_XOR);
            break;
        case '<<':
            compiler.emit(Opcode.LSHIFT);
            break;
        case '>>':
            compiler.emit(Opcode.RSHIFT);
            break;
        case '>>>':
            compiler.emit(Opcode.ZRSHIFT);
            break;
        case '==':
        ...
        case '===':
            ...
            break;
        case '!=':
            compiler.emit(Opcode.NOT_EQUAL);
            break;
        case '!==':
            compiler.emit(Opcode.STRICT_NOT_EQUAL);
            break;
        case '>':
            compiler.emit(Opcode.GREATER_THAN);
            break;
        case '<':
            compiler.emit(Opcode.LESS_THAN);
            break;
        case '>=':
            compiler.emit(Opcode.GREATER_EQUAL);
            break;
        case '<=':
            compiler.emit(Opcode.LESS_EQUAL);
            break;
        case 'in':
            compiler.emit(Opcode.IN);
            break;
        case 'instanceof':
            compiler.emit(Opcode.INSTANCEOF);
            break;
        default: throw new Error(`Compiler error: Unsupported binary operator '${node.operator}'`);
    }
}
export function visitUnaryExpression(compiler, node) {
    ...
    switch (node.operator) {
        ...
        case '!':
            ...
            break;
        case '~':
            compiler.emit(Opcode.BITWISE_NOT);
            break;
        case 'typeof':
            compiler.emit(Opcode.TYPEOF);
            break;
        case 'void':
...
export function visitAssignmentExpression(compiler, node) {
...
export function visitUpdateExpression(compiler, node) {
    if (node.argument.type === 'MemberExpression') {
    ...
    if (resolution.type === 'global') {
        ...
        if (node.operator === '++')
            ...
            throw new Error(`Compiler error: Unsupported update operator '${node.operator}'`);
        compiler.emitStoreVar(resolution);
        if (!node.prefix)
            compiler.emit(Opcode.POP);
    }
}
export function visitLogicalExpression(compiler, node) {
    compiler.visit(node.left);
    compiler.emit(Opcode.DUP);
    if (node.operator === '&&') {
        compiler.emit(Opcode.JUMP_IF_FALSE);
...
export function visitConditionalExpression(compiler, node) {
...
export function visitSequenceExpression(compiler, node) {
...
export function visitMemberExpression(compiler, node) {
...
export function visitCallExpression(compiler, node) {
    if (node.callee.type === 'Super') {
        if (!compiler.currentSuperClass)
            throw new Error("SyntaxError: 'super()' is only valid in derived class constructors.");
        node.arguments.forEach((arg) => compiler.visit(arg));
        ...
        if (thisRes.type !== 'local')
            throw new Error("SyntaxError: 'super' is not allowed in this context.");
    ...
    if (isMemberCall) {
        if (node.callee.object.type === 'Super') {
            if (!compiler.currentSuperClass)
                throw new Error("SyntaxError: 'super' is only valid in derived class methods.");
            const thisRes = compiler.resolveVariable('this');
            if (thisRes.type !== 'local')
                throw new Error("SyntaxError: 'super' is not allowed in this context.");
            compiler.emitLoadVar(thisRes);
        ...
        if (node.callee.computed)
            throw new Error("Compiler error: Computed method calls not supported yet.");
        const methodName = node.callee.property.name;
        const nameIndex = compiler.addConstant(methodName);
        if (hasSpread) {
...
export function visitNewExpression(compiler, node) {
    const hasSpread = node.arguments.some((arg) => arg.type === 'SpreadElement');
    if (hasSpread) {
        compiler.visit(node.callee);
        compiler.emit(Opcode.MAKE_ARRAY);
